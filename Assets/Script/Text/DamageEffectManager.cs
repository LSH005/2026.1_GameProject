...
public class DamageEffectManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (uiCanvas == null)
        {
            ...
            if (uiCanvas == null)
            {
                Debug.LogError("UI 캔버스를 찾을 수 없음.");
            }
        }
    }

    public void ShowDamageText(Vector3 position, string text, Color color, bool isCritical, bool isStatusEffect = false)
    {
        if (textPrefabs == null || uiCanvas == null) return;

        Vector3 screenPos = Camera.main.WorldToScreenPoint(position);
        if (screenPos.z < 0) return; // 카메라 뒤에 있는 경우 무시
    ...
    public void ShowStatusEffect(Vector3 position, string effectName)
    {
        ...
        ShowDamageText(position, effectName, color, false, true);
    }
}
