...
public class CardDisplay : MonoBehaviour
{
    ...
    private void OnMouseUp()
    {
        ...
        {
            Debug.Log($"마나 부족 (필요 : {cardData.manaCost}, 현재 : {CardManager.Instance.playerStats?.currentMana??0})");
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity, enemyLayer))
        {
            ...
            if (enemyStats != null)
            {
                if (cardData.cardType == CardData.CardType.Attack)
                {
                    ...
                    Debug.Log($"{cardData.cardName} 카드로 {cardData.effectAmount} 만큼 공격함");
                    cardUsed = true;
                }
                else
                {
        ...
        else if (Physics.Raycast(ray, out hit, Mathf.Infinity, playerLayer))
        {
            if (CardManager.Instance.playerStats != null)
            {
                if (cardData.cardType == CardData.CardType.Heal)
                {
                    ...
                    Debug.Log($"{cardData.cardName} 카드로 플레이어의 체력을 {cardData.effectAmount} 회복함");
                    cardUsed = true;
                }
            }
            else
            {
                Debug.Log("이 카드는 플레이어에게 사용할 수 없음");
            }
        }
        ...
        {
            transform.position = originalPosition;
            if (CardManager.Instance != null) CardManager.Instance.ArrangeHand();
            return;
        }

        CardManager.Instance.playerStats.UseMana(cardData.manaCost);
        Debug.Log($"{cardData.manaCost} 마나 사용됨. {CardManager.Instance.playerStats.currentMana} 남음.");
